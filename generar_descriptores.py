...
def preparar_csv(ruta):
    """Crea el archivo y su encabezado solamente cuando todavía no existen."""
...
def main():
    # Los argumentos permiten cambiar la cámara o el archivo sin editar el código.
    ...
    preparar_csv(salida)
    # La etiqueta 1 (círculo) queda seleccionada al iniciar.
    etiqueta = 1
    camara = cv2.VideoCapture(args.camara)
    if not camara.isOpened():
        raise RuntimeError("No se pudo abrir la webcam.")
    # Controles para lograr que la figura quede blanca y el fondo negro.
    ...
    try:
        while True:
            # Captura y prepara un fotograma de la webcam.
            ...
            # Se descarta el borde para no guardar accidentalmente todo el fondo.
            contornos = contornos_validos(
                mascara,
                cv2.getTrackbarPos("Area minima", "Controles"),
                descartar_borde=True,
            )
            # El generador guarda una sola muestra: el contorno de mayor área.
            mayor = max(contornos, key=cv2.contourArea, default=None)
            cv2.putText(
                frame,
                f"Etiqueta {etiqueta}: {ETIQUETAS[etiqueta]} | Espacio: guardar",
                (10, 30),
            ...
            if mayor is not None:
                cv2.drawContours(frame, [mayor], -1, (0, 255, 0), 2)
            cv2.imshow("Generador de descriptores", frame)
            cv2.imshow("Mascara", mascara)
            tecla = cv2.waitKey(1) & 0xFF
            # 1, 2 y 3 cambian la clase; espacio agrega la muestra al dataset.
            if tecla == 27:
                break
            if ord("1") <= tecla <= ord("3"):
                etiqueta = tecla - ord("0")
        ...
        # Cierra correctamente la cámara y todas las ventanas.
        camara.release()
